...
def sub01():
    ...
    while True:
        ...
        if m.upper() == 'SAIR':
            ...
            while True:
                try:
                    nome = input('Diga o nome da usuária\n----> ')
                    data = input('\nDiga a data de nascimento da usuária (Formato YYYY-MM-DD)\nEx: 2020-09-09\n----> ')
                    entr = input('\nDiga o dia de entrada da usuária (Formato YYYY-MM-DD)\nEx: 2020-09-09\n----> ')
                    time = input('\nQual o time/grupo da usuária?\n----> ')
                    num = int(input('\nQual o número da camisa da usuária?\n>>>>COLOQUE APENAS NÚMEROS<<<<\n---->'))
                    posi = input('\nQual é a posição da usuária em campo?\n----> ')
                    while True:
                        b = input(f'\nNome: {nome}\nData: {data}\nEntrada: {entr}\nTime: {time}\nCamisa: {num}\nPosição: {posi}\nAs informações ditas estão corretas?\nSim = O programa irá salvar as informações\nNão = O sistema irá reiniciar\n----> ')
                        if b.upper() == 'SIM':

                            with open('users.json', 'w') as wusers, open('users.json', 'r') as rusers:
                                dicto = json.load(rusers)
                                users = dicto['users']
                                while True:
                                ...
                                json.dump(info,wusers)

                except ValueError:
                    m = input('Por favor coloque apenas números\nDê enter para reiniciar a operação\n') #Essa função tem tratamento e resposta apenas quando é colocado um valor que não é número por que temos apenas uma linha que pode dar erro, já que requere apenas números
